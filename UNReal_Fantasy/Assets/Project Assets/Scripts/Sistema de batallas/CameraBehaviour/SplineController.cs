...
public class SplineController : MonoBehaviour
{
	...
	void OnDrawGizmos()
	{
		...
		SetupSplineInterpolator(interp, trans,lookAt);
	...
	void Start()
	{
		...
		if (lookAt == null) {
			lookAt = GetComponentInParent<Transform> ();
			mSplineInterp.AddLookAt(lookAt);
		}
	...
	void SetupSplineInterpolator(SplineInterpolator interp, Transform[] trans,Transform mLookAt)
	{
		interp.Reset();
	...
	Transform[] GetTransforms()
	{
		if (SplineRoot != null)
		{
			...
			//Por ultimo, buscamos la transformacion de la camara del protagonista, y la dejamos de ultimo
			transforms.Add(GameObject.FindGameObjectWithTag("MainCamera").GetComponent<Transform>());
	...
	void FollowSpline()
	{
		...
		{
			SetupSplineInterpolator(mSplineInterp, mTransforms,lookAt);
